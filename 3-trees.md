...
The common operation *insert*, *remove*, and *contains* all have an algorithmic efficiency of O(log n). We can see this in the way these methods are implemented such as the insert method described above. \
The traverse methods along with the height are all O(n). This is because in these three methods each node is visited once. This takes place with the height function as it must determine and compare the heights of all subtrees in order to find the maximum length of a root to a leaf.\
Finally size and empty are O(1) similar to our other data structures we have discussed.
## VI. Example: Sorting Books I
...
```
The BinarySearchTree will also contain methods for traversing the data in the tree. This works by going to the left-most node and working to the right. The reverse function is built similarly, but works from the right-most node to the left.
```c#
...
**Hint**: You can compare strings in C# using the ```CompareTo()``` method.
```c#
myString.CompareTo(otherString);
// Returns -1 if myString comes before otherString.
// Returns 0 if the strings match.
// Returns 1 if otherString comes before myString.
```
